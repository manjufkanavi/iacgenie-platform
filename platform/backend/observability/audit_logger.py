...
class AuditLogger:
    def log_event(self, event_type: str, details: dict = None, user_id: str = None):
        logger.info(f"[AUDIT] {event_type}: {details or {}}")

    def log_pipeline_run(self, pipeline_id: str, phase: str, status: str, user_id: str = None):
        logger.info(f"[AUDIT] Pipeline {pipeline_id}: {phase} -> {status}")

    def log_error(self, error: Exception, context: dict = None):
        logger.error(f"[AUDIT] Error: {error} {context or {}}")

    def log_pipeline_event(self, session_id: str, event_type: str, details: dict = None, user_id: str = None):
        logger.info(f"[AUDIT] Pipeline {session_id}: {event_type}")

    def log_human_intervention(self, session_id: str, action: str, details: dict = None, user_id: str = None):
        logger.info(f"[AUDIT] Human intervention on {session_id}: {action}")

    def log_approval_event(self, session_id: str, approval_type: str, action: str, details: dict = None, user_id: str = None):
        logger.info(f"[AUDIT] Approval on {session_id}: {approval_type} -> {action}")

    async def async_log(self, event_type: str, details: dict = None):
